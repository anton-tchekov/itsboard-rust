...
pub struct WeakList<T> where T: Ord {
	head: Link<T>,
	tail: Link<T>,
	count: usize
}
...
type BackLink<T> = Option<Weak<RefCell<Node<T>>>>;
...
impl<T> WeakList<T> where T: Ord {
	...
	pub fn insert(&mut self, elem: T) {
		...
		if node.clone().as_ref().borrow().elem < self.head.as_ref().unwrap().borrow().elem {
			...
			self.head.as_ref().unwrap().as_ref().borrow_mut().prev = Some(Rc::downgrade(&node));
			self.head = Some(node.clone());
			return;
		}
		...
		if node.clone().as_ref().borrow().elem > self.tail.as_ref().unwrap().borrow().elem {
			...
			node.clone().as_ref().borrow_mut().prev = Some(Rc::downgrade(&self.tail.clone().unwrap()));
			self.tail.as_ref().unwrap().as_ref().borrow_mut().next = Some(node.clone());
			self.tail = Some(node);
			return;
		}
...
pub struct IntoIter<T>(WeakList<T>) where T: Ord;
